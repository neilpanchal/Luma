...
public class Luma {
    ...
    public Luma(int clusterCount_, int quality_,  double minL_, double maxL_,
        ...
        // Create random data points in the restricted domain. This method does not allow for constant domain size. The total number of data points is proportional to the input ranges of luma, chroma and hue. As a consequence, the processing time is not constant & difficult to predict. Therefore, it has been deprecated and replaced by the 'createDomain' method.

        /*
        nodeList = new ArrayList<Node>();
        nodeList = createDomain((int)mapRange(quality, 0, 100, 1000, 7500), minL_, maxL_, minC_, maxC_, minH_, maxH_);
        */

        nodeList = new ArrayList<Node>();
        nodeList = createDomain((int)mapRange(quality, 0.0F, 100.0, 1000.0, 7500.0), minL_, maxL_, minC_, maxC_, minH_, maxH_);
    ...
    private class Centroid {
        ...
        public Centroid (Centroid toClone) {
            ...
            this.lumaChroma = new Chroma(ColorSpace.LCH, lchComp[0], lchComp[1], lchComp[2]);
        }
    ...
    // DATASET CLASS ---- ///////////////////////////////////////////////////////////////
    // This class holds all points in the Domain and the ID of the closest cluster centroid. The ID field is synced with the j-index of the centroidList ArrayList.
    ...
    public void kMeansClustering() {
        ...
        while(!converged) {
            ...
            // Assign each data point to its nearest cluster centroid. The centroid ID assigned to each data point is the current j-index of the centroidList ArrayList.
            for (int i = 0; i < nodeList.size(); i++) {
                double minDistance = LARGE_NUMBER;

                for(int j = 0; j < centroidList.size(); j++) {
                    double distance = computeDistance(nodeList.get(i), centroidList.get(j));
            ...
            for (int j = 0; j < centroidList.size(); j++) {
                ...
                if (count!=0) {
                    ...
                    Centroid tempCentroid = new Centroid(kMean[0], kMean[1], kMean[2]);

                    // Check if the new location of the centroid is within the bounds

                    if (validChroma(tempCentroid.get())) {

                        convergenceTest[j] = checkConvergence(tempCentroid, centroidList.get(j));
                        ...
                        */
                        ...
                        Centroid randomCentroid = new Centroid(getRandom(this.minL,this.maxL), getRandom(this.minC,this.maxC), getRandom(this.minH, this.maxH));
                    ...
                    for (int i = 0; i < nodeList.size(); i++) {
                        ...
                        double distance = computeDistance(nodeList.get(i), currentCluster);
                        if (distance < minDistance) {
                            minDistance = distance;
                            closest = i;
                        }
                    ...
                    // Clone the data point and create a centroid at the new location. Cloning is important otherwise the centroid will only get copy the reference to the Node object and move the data point itself in subsequent iterations. If the cluster has only one data point, it will not converge.

                    Centroid closestCentroid = new Centroid(nodeList.get(closest));
    ...
    /////////////////////////////////////////////////////////////////////////////////////
    private ArrayList<Node> createDomain(int size_, double minL_, double maxL_, double minC_, double maxC_, double minH_, double maxH_) {
        ...
        for (int i = 0; i < size_; i++) {
            ...
            while(!foundValidChroma) {
                Node tempData = new Node(getRandom(this.minL,this.maxL), getRandom(this.minC,this.maxC), getRandom(this.minH, this.maxH));
    ...
    private ArrayList<Centroid> createClusters(int clusterCount_) {
        ...
        for (int i = 0; i < clusterCount_; i++) {
            ...
            while(!foundValidCentroid) {
                ...
                Centroid tempCentroid = new Centroid(getRandom(this.minL,this.maxL), getRandom(this.minC,this.maxC), getRandom(this.minH, this.maxH));
    ...
    /*  ---- DEPRECATED -----------------------------------------------------------------
    private ArrayList<Node> createDomain(double minL_, double maxL_, double minC_, double maxC_, double minH_, double maxH_) {
    ...
    */
    ...
    private boolean validRange(Chroma chromaColor_) {
        return (chromaColor_.getLCH(Channel.L) > this.minL)
            ...
            && (chromaColor_.getLCH(Channel.H) > this.minH)
            && (chromaColor_.getLCH(Channel.H) < this.maxH);
    }


    private boolean checkConvergence(Centroid currentCentroid_, Centroid prevCentroid_) {
    ...
    private static double computeDistance(Centroid point1_, Centroid point2_) {
        return Math.sqrt(
            Math.pow(point1_.get().getLCH(Channel.L) - point2_.get().getLCH(Channel.L), 2) +
            Math.pow(point1_.get().getLCH(Channel.C) - point2_.get().getLCH(Channel.C), 2) +
            Math.pow(point1_.get().getLCH(Channel.H) - point2_.get().getLCH(Channel.H), 2));
    ...
    private static double mapRange(double value_, double inputMin_, double inputMax_, double outputMin_, double outputMax_) {
